...
class SendApprovelEmailNotification extends Notification
{
    ...
    public function getMessage()
    {
        ...
        // $email_content = SystemEmail::find(1);
         if(!empty($this->user->program->id)){
            $email_content = SystemEmail::where('type', '=', 2)->where('name_id', '=', 1)
        ...
        $subject = config('app.name') . ':  Registration Completion - ' . $this->user->program->name ;
        $content = "";
        ...
        return (new MailMessage)
            ...
            // ->line('Kindly be informed that your application to join the '. $this->user->program.' programme at The International Digital University (IDU) has been approved. The programme will start in January, 2021, the exact starting date will be communicated with you soon.')
