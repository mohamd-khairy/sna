    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jobs.update", [$job->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $job->description) !!}</textarea>
            ...
            <div class="form-group">
                <label for="responsibilities">{{ trans('cruds.job.fields.responsibilities') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('responsibilities') ? 'is-invalid' : '' }}" name="responsibilities" id="responsibilities">{!! old('responsibilities', $job->responsibilities) !!}</textarea>
                @if($errors->has('responsibilities'))
                    <div class="invalid-feedback">
                        {{ $errors->first('responsibilities') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.responsibilities_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="requirements">{{ trans('cruds.job.fields.requirements') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('requirements') ? 'is-invalid' : '' }}" name="requirements" id="requirements">{!! old('requirements', $job->requirements) !!}</textarea>
                @if($errors->has('requirements'))
                    <div class="invalid-feedback">
                        {{ $errors->first('requirements') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.requirements_helper') }}</span>
            </div>
            <div class="form-group">
