            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-User">
                <thead>
                    ...
                    <tr>
                        ...
                        <td>
                            <input class="search"  style="width: 100%" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                             <input class="search" style="width: 100%" type="text" placeholder="{{ trans('global.search') }}">
                        </td>
                        ...
                        <td>
                            <input class="search" type="text" style="width: 100%" placeholder="{{ trans('global.search') }}">
                        </td>
                        <td>
                            <select class="search" strict="true">
                                <option value>{{ trans('global.all') }}</option>
                                @foreach(App\Models\User::DEGREE_SELECT as $key => $item)
                    ...
                    @foreach($users as $key => $user)
                        <tr data-entry-id="{{ $user->id }}">
                            ...
                            <td>
                                ...
                                @can('user_verify')
                                    @if($user->verified == 0)
                                        <form action="{{ route('admin.users.verify') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                            <input type= "hidden" name="_method" value="POST">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <input type="hidden" name="id" value="{{ $user->id }}">
                                            <input type="submit" class="btn btn-xs btn-success" value="verify">
                                        </form>
                                    @endif
                                @endcan
                                @can('user_delete')
